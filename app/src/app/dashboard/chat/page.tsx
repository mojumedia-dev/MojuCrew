...
const STEPS: WizardStep[] = [
  ...
  },
  {
    ...
    render: (data, update) => (
      <div className="space-y-5">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Chat tone</label>
          <div className="grid grid-cols-2 gap-3">
            {TONES.map((t) => (
              <button key={t} type="button" onClick={() => update({ tone: t })} className={`py-2.5 px-4 rounded-lg border text-sm font-medium transition-colors ${data.tone === t ? "bg-black text-white border-black" : "border-gray-200 text-gray-600 hover:border-gray-400"}`}>{t}</button>
            ))}
          </div>
        </div>
        <div>
    ...
    render: (data, update) => {
      ...
      return (
        <div className="space-y-4">
          <p className="text-sm text-gray-500">Add questions your customers commonly ask. MojuChat will answer these automatically.</p>
          {faqs.map((faq, i) => (
  ...
  },
  {
    ...
    render: (data) => {
      ...
      return (
        <div className="space-y-5">
          <p className="text-sm text-gray-500">Paste this snippet before the <code className="bg-gray-100 px-1 rounded text-xs">&lt;/body&gt;</code> tag on your website.</p>
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-xl border p-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><span className="text-gray-400 block mb-0.5">Website</span><span className="text-gray-900 font-medium">{config.websiteUrl as string ?? "—"}</span></div>
              ...
              <div className="col-span-2"><span className="text-gray-400 block mb-0.5">FAQs</span><span className="text-gray-900 font-medium">{((config.faqs as unknown[]) ?? []).length} entries</span></div>
            </div>
          </div>
