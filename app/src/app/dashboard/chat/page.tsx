      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            placeholder="Hi! I'm the assistant for [Business]. How can I help you today?"
      ...
      return (
        <div className="space-y-5">
          <p className="text-sm text-gray-500">
            Paste this snippet before the <code className="bg-gray-100 px-1 rounded text-xs">&lt;/body&gt;</code> tag on your website. MojuChat will appear instantly.
          </p>
          <div className="bg-gray-900 rounded-lg p-4 relative">
            ...
            <button
              ...
              className="absolute top-3 right-3 text-xs text-gray-400 hover:text-white bg-gray-700 px-2 py-1 rounded"
            >
              Copy
            </button>
          </div>
          <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 text-sm text-blue-700">
            Your full embed key will be generated after account activation. This is a preview.
          </div>
        </div>
      );
    },
...
export default function MojuChatPage() {
  ...
  useEffect(() => {
    setConfig(getBotConfig("chat"));
    setLoaded(true);
  }, []);

  const handleComplete = (data: Record<string, unknown>) => {
    saveBotConfig("chat", data);
    setConfig(getBotConfig("chat")!);
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-xl border p-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <span className="text-gray-900 font-medium">{config.websiteUrl as string ?? "—"}</span>
              </div>
              <div>
                <span className="text-gray-400 block mb-0.5">Industry</span>
                <span className="text-gray-900 font-medium">{config.industry as string ?? "—"}</span>
              </div>
              <div>
              ...
              <div className="col-span-2">
                <span className="text-gray-400 block mb-0.5">FAQs</span>
                <span className="text-gray-900 font-medium">{((config.faqs as unknown[]) ?? []).length} entries</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-900 rounded-xl p-6">
            ...
            <pre className="text-xs text-green-400 whitespace-pre-wrap break-all">
              {`<script src="https://cdn.mojucrew.com/chat.js" data-key="YOUR_KEY" data-name="${config.businessName ?? "Your Business"}" data-tone="${((config.tone as string) ?? "friendly").toLowerCase()}"></script>`}
            </pre>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={() => { clearBotConfig("chat"); setConfig(null); }}
