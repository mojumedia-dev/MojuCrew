...
export default function MojuContentPage() {
  ...
  useEffect(() => { setConfig(getBotConfig("content")); setLoaded(true); }, []);

  const handleComplete = (data: Record<string, unknown>) => {
    saveBotConfig("content", data);
    setConfig(getBotConfig("content")!);
      ...
      {config && !reconfiguring ? (
        <div className="space-y-5">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => { clearBotConfig("content"); setConfig(null); }} className="px-5 py-2 text-sm text-red-500 border border-red-100 rounded-lg hover:bg-red-50 transition-colors">Deactivate</button>
          </div>
        </div>
      ) : (
