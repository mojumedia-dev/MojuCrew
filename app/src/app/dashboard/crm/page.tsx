...
const STEPS: WizardStep[] = [
    ...
    render: (data, update) => {
      ...
      return (
        <div className="space-y-4">
          <p className="text-sm text-gray-500">Customize your sales pipeline. MojuCRM moves leads through these stages automatically.</p>
          {stages.map((stage, i) => (
  ...
  },
  {
    ...
    render: (data, update) => (
      <div className="space-y-5">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Number of follow-up attempts</label>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Cadence between follow-ups</label>
          <div className="grid grid-cols-3 gap-2">
            {CADENCES.map((c) => (
              <button key={c} type="button" onClick={() => update({ cadence: c })} className={`py-2 rounded-lg border text-sm font-medium transition-colors ${data.cadence === c ? "bg-black text-white border-black" : "border-gray-200 text-gray-600 hover:border-gray-400"}`}>{c}</button>
            ))}
          </div>
        </div>
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">Use [Name] as a placeholder for the contact's name.</p>
        </div>
      </div>
    ),
  },
  {
    ...
    render: (data, update) => (
      <div className="space-y-5">
        <p className="text-sm text-gray-500">Connect your email so MojuCRM can log conversations and send follow-ups automatically.</p>
...
export default function MojuCRMPage() {
  ...
  useEffect(() => { setConfig(getBotConfig("crm")); setLoaded(true); }, []);

  const handleComplete = (data: Record<string, unknown>) => {
    saveBotConfig("crm", data);
    setConfig(getBotConfig("crm")!);
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-xl border p-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><span className="text-gray-400 block mb-0.5">Industry</span><span className="text-gray-900 font-medium">{config.industry as string ?? "—"}</span></div>
              <div><span className="text-gray-400 block mb-0.5">Follow-ups</span><span className="text-gray-900 font-medium">{config.followUpCount as string ?? "—"} · {config.cadence as string ?? "—"}</span></div>
              <div><span className="text-gray-400 block mb-0.5">Email</span><span className="text-gray-900 font-medium">{config.emailProvider as string ?? "Not connected"}</span></div>
              <div className="col-span-2"><span className="text-gray-400 block mb-0.5">Pipeline</span><span className="text-gray-900 font-medium">{((config.stages as string[]) ?? []).join(" → ")}</span></div>
            </div>
          </div>
          <div className="flex gap-3">
            ...
            <button onClick={() => { clearBotConfig("crm"); setConfig(null); }} className="px-5 py-2 text-sm text-red-500 border border-red-100 rounded-lg hover:bg-red-50 transition-colors">Deactivate</button>
          </div>
        </div>
      ) : (
