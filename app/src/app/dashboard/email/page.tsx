...
const STEPS: WizardStep[] = [
  {
    ...
    render: (data, update) => (
      <div className="space-y-5">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Send emails from</label>
          <input type="email" value={(data.fromEmail as string) ?? ""} onChange={(e) => update({ fromEmail: e.target.value })} placeholder="hello@yourbusiness.com" className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-black" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Reply-to email</label>
          <input type="email" value={(data.replyTo as string) ?? ""} onChange={(e) => update({ replyTo: e.target.value })} placeholder="Same as above (or a different address)" className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-black" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">What do you want to tell your subscribers?</label>
  ...
  },
  {
    ...
    render: (data, update) => (
      <div className="space-y-4">
        <p className="text-sm text-gray-500">What types of campaigns will you run? MojuEmail will tailor its writing to these goals.</p>
      ...
      <div className="space-y-5">
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {TONES.map((t) => (
              <button key={t} type="button" onClick={() => update({ tone: t })} className={`px-4 py-2 rounded-lg border text-sm font-medium transition-colors ${data.tone === t ? "bg-black text-white border-black" : "border-gray-200 text-gray-600 hover:border-gray-400"}`}>{t}</button>
            ))}
          </div>
        </div>
        <div>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Preferred send day</label>
  ...
  },
  {
    ...
    render: (data, update) => (
      <div className="space-y-5">
        <p className="text-sm text-gray-500">Connect your existing email list or start fresh. MojuEmail will manage your subscribers and unsubscribes automatically.</p>
        ...
        <div className="flex items-center justify-between py-3 border border-gray-100 rounded-lg px-4">
          <div>
            <p className="text-sm font-medium text-gray-700">Review emails before sending</p>
            <p className="text-xs text-gray-400">Approve each campaign draft before MojuEmail sends it</p>
          </div>
...
export default function MojuEmailPage() {
  ...
  useEffect(() => { setConfig(getBotConfig("email")); setLoaded(true); }, []);

  const handleComplete = (data: Record<string, unknown>) => {
    saveBotConfig("email", data);
    setConfig(getBotConfig("email")!);
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-xl border p-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><span className="text-gray-400 block mb-0.5">From email</span><span className="text-gray-900 font-medium">{config.fromEmail as string ?? "—"}</span></div>
              <div><span className="text-gray-400 block mb-0.5">Frequency</span><span className="text-gray-900 font-medium">{config.frequency as string ?? "—"} · {config.sendDay as string ?? "—"}s</span></div>
              <div><span className="text-gray-400 block mb-0.5">Tone</span><span className="text-gray-900 font-medium">{config.tone as string ?? "—"}</span></div>
              <div><span className="text-gray-400 block mb-0.5">List provider</span><span className="text-gray-900 font-medium">{config.listProvider as string ?? "Not connected"}</span></div>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => { clearBotConfig("email"); setConfig(null); }} className="px-5 py-2 text-sm text-red-500 border border-red-100 rounded-lg hover:bg-red-50 transition-colors">Deactivate</button>
          </div>
        </div>
      ) : (
