...
const STEPS: WizardStep[] = [
  {
    ...
    render: (data, update) => (
      <div className="space-y-5">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Where do your leads come from?</label>
          <div className="space-y-2">
            {LEAD_SOURCES.map((s) => {
              ...
              return (
                ...
                }} className={`w-full flex items-center gap-3 py-2.5 px-4 rounded-lg border text-sm font-medium text-left transition-colors ${active ? "bg-black text-white border-black" : "border-gray-200 text-gray-600 hover:border-gray-400"}`}>
  ...
  },
  {
    ...
    render: (data, update) => (
      <div className="space-y-5">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Follow-up channel</label>
          <div className="grid grid-cols-3 gap-3">
            {CHANNELS.map((c) => (
              <button key={c} type="button" onClick={() => update({ channel: c })} className={`py-2.5 rounded-lg border text-sm font-medium transition-colors ${data.channel === c ? "bg-black text-white border-black" : "border-gray-200 text-gray-600 hover:border-gray-400"}`}>{c}</button>
            ))}
          </div>
        </div>
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {SEQUENCE_LENGTHS.map((s) => (
              <button key={s} type="button" onClick={() => update({ sequenceLength: s })} className={`py-2.5 rounded-lg border text-sm font-medium transition-colors ${data.sequenceLength === s ? "bg-black text-white border-black" : "border-gray-200 text-gray-600 hover:border-gray-400"}`}>{s}</button>
            ))}
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Delay between messages</label>
          <div className="flex flex-wrap gap-2">
            {DELAYS.map((d) => (
              <button key={d} type="button" onClick={() => update({ delay: d })} className={`px-4 py-2 rounded-lg border text-sm font-medium transition-colors ${data.delay === d ? "bg-black text-white border-black" : "border-gray-200 text-gray-600 hover:border-gray-400"}`}>{d}</button>
            ))}
          </div>
        </div>
      </div>
  ...
  },
  {
    ...
    render: (data, update) => (
      <div className="space-y-5">
        <p className="text-sm text-gray-500">Write your first follow-up message. MojuLeads will generate the rest of the sequence based on your tone and goal.</p>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">First follow-up message</label>
          ...
          <p className="text-xs text-gray-400 mt-1">Use [Name] and [Business] as placeholders.</p>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">What&apos;s your goal for these leads?</label>
  ...
  },
  {
    ...
    render: (data, update) => (
      <div className="space-y-5">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Alert me when a lead replies</label>
        ...
        <div className="flex items-center justify-between py-3 border border-gray-100 rounded-lg px-4">
          <div>
            <p className="text-sm font-medium text-gray-700">Pause sequence when lead replies</p>
            <p className="text-xs text-gray-400">Stop automated messages once the lead responds</p>
          </div>
...
export default function MojuLeadsPage() {
  ...
  useEffect(() => { setConfig(getBotConfig("leads")); setLoaded(true); }, []);

  const handleComplete = (data: Record<string, unknown>) => {
    saveBotConfig("leads", data);
    setConfig(getBotConfig("leads")!);
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-xl border p-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><span className="text-gray-400 block mb-0.5">Channel</span><span className="text-gray-900 font-medium">{config.channel as string ?? "—"}</span></div>
              <div><span className="text-gray-400 block mb-0.5">Sequence</span><span className="text-gray-900 font-medium">{config.sequenceLength as string ?? "—"} · every {config.delay as string ?? "—"}</span></div>
              <div><span className="text-gray-400 block mb-0.5">Goal</span><span className="text-gray-900 font-medium">{config.goal as string ?? "—"}</span></div>
              <div><span className="text-gray-400 block mb-0.5">Pause on reply</span><span className="text-gray-900 font-medium">{config.pauseOnReply !== false ? "Yes" : "No"}</span></div>
              <div><span className="text-gray-400 block mb-0.5">Alert email</span><span className="text-gray-900 font-medium">{config.alertEmail as string ?? "—"}</span></div>
            </div>
          </div>
          <div className="flex gap-3">
            ...
            <button onClick={() => { clearBotConfig("leads"); setConfig(null); }} className="px-5 py-2 text-sm text-red-500 border border-red-100 rounded-lg hover:bg-red-50 transition-colors">Deactivate</button>
          </div>
        </div>
      ) : (
