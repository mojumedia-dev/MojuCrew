...
const STEPS: WizardStep[] = [
  {
    ...
    render: (data, update) => (
      <div className="space-y-5">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">What products or services do you offer?</label>
  ...
  },
  {
    ...
    render: (data, update) => (
      <div className="space-y-4">
        <p className="text-sm text-gray-500">When should MojuUpsell reach out to past customers?</p>
  ...
  },
  {
    ...
    render: (data, update) => (
      <div className="space-y-5">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">What kind of offer will you send?</label>
          <div className="space-y-2">
            {DISCOUNT_TYPES.map((d) => (
              <button key={d} type="button" onClick={() => update({ discountType: d })} className={`w-full py-2.5 px-4 rounded-lg border text-sm font-medium text-left transition-colors ${data.discountType === d ? "bg-black text-white border-black" : "border-gray-200 text-gray-600 hover:border-gray-400"}`}>{d}</button>
            ))}
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Offer message template</label>
          ...
          <p className="text-xs text-gray-400 mt-1">Use [Name] and [Link] as placeholders.</p>
        </div>
      </div>
    ),
  },
  {
    ...
    render: (data, update) => (
      <div className="space-y-5">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Send upsell offers via</label>
        ...
        <div className="flex items-center justify-between py-3 border border-gray-100 rounded-lg px-4">
          <div>
            <p className="text-sm font-medium text-gray-700">Only contact customers once per period</p>
            <p className="text-xs text-gray-400">Prevent sending multiple offers too close together</p>
          </div>
...
export default function MojuUpsellPage() {
  ...
  useEffect(() => { setConfig(getBotConfig("upsell")); setLoaded(true); }, []);

  const handleComplete = (data: Record<string, unknown>) => {
    saveBotConfig("upsell", data);
    setConfig(getBotConfig("upsell")!);
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-xl border p-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><span className="text-gray-400 block mb-0.5">Offer type</span><span className="text-gray-900 font-medium">{config.discountType as string ?? "—"}</span></div>
              <div><span className="text-gray-400 block mb-0.5">Throttle</span><span className="text-gray-900 font-medium">{config.throttle !== false ? "On" : "Off"}</span></div>
              <div className="col-span-2"><span className="text-gray-400 block mb-0.5">Triggers</span><span className="text-gray-900 font-medium">{((config.triggers as string[]) ?? []).join(", ") || "—"}</span></div>
            </div>
          </div>
          <div className="flex gap-3">
            ...
            <button onClick={() => { clearBotConfig("upsell"); setConfig(null); }} className="px-5 py-2 text-sm text-red-500 border border-red-100 rounded-lg hover:bg-red-50 transition-colors">Deactivate</button>
          </div>
        </div>
      ) : (
